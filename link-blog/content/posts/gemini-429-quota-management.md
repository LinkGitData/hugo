...
在我們正式環境的 `prod-enterprise-rd` 專案中，負責「圖片辨識」與微調模型預測的服務，近期頻繁遭遇以下錯誤：
...
Google 開發者論壇近 12 個月共 49 篇「429 Resource Exhausted」相關討論，歸納五個共同樣態：

1. **量小照樣中獎**：34 個請求就 429、配額剩 99.5% 仍被拒——低用量 429 是最大宗抱怨，官方回覆一律指向 DSQ＋重試＋PT。
2. **Preview／圖像模型最嚴重**：用量層級不保護預覽版模型，純靠搶；我們用的 2.5-flash 是 GA 模型，處境相對好。
3. **全域端點與重試都非保證**：有案例重試 10 次、跨 10 分鐘仍全數失敗——容量嚴重吃緊時，唯一的保證是 PT。
4. **新專案待遇更差**：同一模型舊專案正常、新專案持續 429：層級看消費歷史。
5. **2026 年 2–4 月起整體惡化**：討論量激增、出現數起區域級事件，與我們 7/7–7/9 的惡化趨勢吻合——共用池只會越來越擠。
...
3. **批次 / Flex PayGo (成本 5 折)**：將可延後流量進佇列；Flex 接受 30 分內回應，換取彈性（適用微調預測等非即時工作）。
...
- **中期 (本季)**：若是對即時性要求不高的功能，改走批次 / Flex PayGo；並與資安單位確認後，建立合規邊界內的多區域 failover。
